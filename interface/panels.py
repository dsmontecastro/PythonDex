...
class InformationPanel():
    ...
    def initialize_components(self):
        ...
        self.background_1_image = p.image.load('resources/backgrounds/{}.png'.format(self.type_1))

        # Check if type_2 is a string or a NaN float
        if isinstance(self.type_2, str):
            self.background_2_image = p.image.load('resources/backgrounds/{}.png'.format(self.type_2))
        else:
            self.background_2_sprite.image = p.image.load('resources/backgrounds/blank.png')

        self.background_1_sprite = p.sprite.Sprite(self.background_1_image, batch=self.back)
        self.background_2_sprite = p.sprite.Sprite(self.background_2_image, batch=self.back)
        ...
        try:
            self.pokemon_image = p.image.load('resources/pokemon/{}.jpg'.format(self.name.lower()))
        except(FileNotFoundError):
            pass
        try:
            self.pokemon_image = p.image.load('resources/pokemon/{}.png'.format(self.name.lower()))
        except(FileNotFoundError):
            raise FileNotFoundError("Invalid pokemon name.")
        self.pokemon_image.anchor_x = self.pokemon_image.width
        self.pokemon_sprite = p.sprite.Sprite(self.pokemon_image, self.background_1_sprite.width, self.background_1_sprite.width, batch=self.front)

        # Load labels
        self.label_name = p.text.Label("Name: {}".format(self.data.name), font_name="Power Clear", x=30, y=440, font_size=15, batch=self.front)
        self.label_abilities = p.text.Label("Abilities: {}".format(', '.join(self.data.abilities)), font_name="Power Clear", x=30, y=420, font_size=12, batch=self.front)
        self.label_weight = p.text.Label("WT: {} kg".format(self.data.weight), font_name="Power Clear", x=30, y=400, font_size=12, batch=self.front)
        self.label_height = p.text.Label("HT: {} m".format(self.data.height), font_name="Power Clear", x=30, y=380, font_size=12, batch=self.front)

        self.width = self.background_1_image.width
        self.height = self.background_1_image.height

        self.hexagon_image = p.image.load('resources/tiles/stat_wheel.png')
        self.hexagon_image.anchor_x = int(self.hexagon_image.width/2)
        self.hexagon_image.anchor_y = int(self.hexagon_image.height/2)
        self.hexagon_sprite = p.sprite.Sprite(self.hexagon_image, self.background_1_sprite.width/2, self.hexagon_image.height/2 + 25, batch=self.front)

        self.stats_image = p.image.load('resources/tiles/stat_names.png')
        self.stats_image.anchor_x = int(self.stats_image.width/2) + 10
        self.stats_image.anchor_y = int(self.stats_image.height/2)
        self.stats_sprite = p.sprite.Sprite(self.stats_image, self.hexagon_sprite.x, self.hexagon_sprite.y, batch=self.front)
    ...
    def update_background_sprite(self):
        ...
        self.background_1_sprite.image = p.image.load('resources/backgrounds/{}.png'.format(self.type_1))

        # Check if type_2 is a string or a NaN float
        # Made as an artifact of using pandas for data loading
        if isinstance(self.type_2, str):
            self.background_2_sprite.image = p.image.load('resources/backgrounds/{}.png'.format(self.type_2))
        else:
            self.background_2_sprite.image = p.image.load('resources/backgrounds/blank.png')
    ...
    def update_pokemon_sprite(self):
        try:
            self.pokemon_image = p.image.load('resources/pokemon/{}.jpg'.format(self.data.name.lower()))
        except(FileNotFoundError):
            pass
        try:
            self.pokemon_image = p.image.load('resources/pokemon/{}.png'.format(self.data.name.lower()))
        except(FileNotFoundError):
           pass
        self.pokemon_image.anchor_x = self.pokemon_image.width
    ...
    def draw_self(self):
        self.back.draw()
        self.front.draw()
        p.graphics.draw(4, p.gl.GL_QUADS, ('v2f', (self.background_1_sprite.width, self.background_1_sprite.y,
...
class Browser():
    ...
    def __init__(self, database, favorites, x, y):
        ...
        # Preload image tiles
        self.select_active_image = p.image.load('resources/tiles/select_active.png')
        ...
        self.select_inactive_image = p.image.load('resources/tiles/select_inactive.png')
        ...
        self.star_active_image = p.image.load('resources/tiles/fav_active.png')
        ...
        self.star_inactive_image = p.image.load('resources/tiles/fav_inactive.png')
        ...
        for i in range(self.cycle_max):
            self.tiles.append(p.sprite.Sprite(self.select_inactive_image, batch=self.back, x=self.tile_pos[0], y=self.tile_pos[1]-self.tile_pos[2]*i))
            self.stars.append(p.sprite.Sprite(self.star_inactive_image, batch=self.back, x=self.star_pos[0], y=self.star_pos[1]-self.star_pos[2]*i))
            ...
            self.texts.append(p.text.Label('{} - {}'.format(processes.add0(self.database[i].index), self.database[i].name)
...
class SearchPanel():
    ...
    def __init__(self, database, x, y):
        ...
        self.database = database
        self.search_string = ""
        self.label = p.text.Label(text="Search...", font_name="Power Clear", font_size=14, x=380, y=440, batch=self.front, color=Color.BLACK+tuple([255]), anchor_x="left", anchor_y="center")
...
class ScrollBar():
    def __init__(self, database, up=0, down=0, left=0, right=0):
        ...
        self.bar_img = p.image.load('resources\\tiles\\scroll_bar.png')
    ...
    def draw_self(self):
        p.graphics.draw(4, p.gl.GL_QUADS, ('v2f', self.square),
                                          ('c3B', Color.GREY * 4))
        self.bar.draw()
